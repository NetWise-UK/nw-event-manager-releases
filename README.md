# nw-event-manager-releases

This repo holds built releases (zip files) of the **NW Event Manager** WordPress
plugin. It has no source code — that lives in the private `nw-event-manager`
repo. Client sites check here for updates.

`nw-event-manager.json` is the file every client site actually polls. It is
